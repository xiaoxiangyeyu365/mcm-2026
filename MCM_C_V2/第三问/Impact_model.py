...
def make_model(train_df: pd.DataFrame, y_col: str, features: List[str]):
    ...
    numeric = [c for c in ["celebrity_age_during_season", "season_progress"] if c in features]
    passthrough = [c for c in features if c.startswith("ind_")]
...
def main() -> None:
    ...
    for spec in specs:
        ...
        age_coef = coef_all.loc[coef_all["feature"] == "celebrity_age_during_season", "coef"]
        age_val = float(age_coef.iloc[0]) if len(age_coef) else float("nan")
        ce = pd.concat(
            [
                pd.DataFrame([{"name": "age", "coef": age_val}]),
                industry_effects[spec.name].copy(),
            ],
            ignore_index=True,
        )
    ...
    # celebrity characteristics outputs
